...
import { 
  ...
  CheckCircle, Clock, Database as DbIcon, MessageSquare, Loader
} from 'lucide-react';
...
const ServiceDetailView = ({ user, profile }: { user: any, profile: UserProfile | null }) => {
  ...
  const handleBooking = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        const { data: providerProfileData } = await db.from('users').select('mobile_number').eq('uid', service?.ownerId).single();
        if (providerProfileData?.mobile_number) {
          const alertMsg = `New Service Query for ${service?.name}!\nVisitor: ${visitorName}\nMobile: ${visitorMobile}\nDate: ${date}\nMessage: ${message || 'No message'}`;
          sendWhatsAppAlert(providerProfileData.mobile_number, alertMsg);
        }
      } catch (waErr) {
        ...
        <div className="bg-white p-6 md:p-10 rounded-[2.5rem] border border-gray-100 shadow-sm flex flex-col md:flex-row items-center md:items-start space-y-6 md:space-y-0 md:space-x-8">
          <div className="flex-1 text-center md:text-left min-w-0">
             ...
             <div className="flex items-center justify-center md:justify-start space-x-4">
               ...
               <div className="text-left">
                 <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-none mb-1 font-sans">Provider</p>
          ...
          <div className="shrink-0 w-full md:w-auto">
             <div className="bg-gradient-to-br from-purple-600 to-indigo-700 text-white p-6 md:p-8 rounded-[2rem] shadow-xl shadow-purple-100 text-center min-w-[220px]">
                <span className="block text-[10px] font-black uppercase tracking-[0.2em] mb-1 opacity-80 underline underline-offset-4 font-sans">Package Starts</span>
                <span className="text-2xl md:text-3xl font-black">{service.priceRange}</span>
                <span className="block text-xs font-bold mt-1 opacity-80 font-sans">customizable pricing</span>
             </div>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100 flex flex-wrap gap-8 justify-around">
           <div className="flex items-center space-x-4">
              ...
              <div>
                 ...
                 <span className="text-xl font-black text-gray-900 leading-none">{service.rating > 0 ? service.rating : 'New'} Stars</span>
              </div>
           </div>
           <div className="flex items-center space-x-4">
              ...
              <div>
                 ...
                 <span className="text-xl font-black text-gray-900 leading-none">{service.reviewCount || 0} Total</span>
              </div>
           </div>
           <div className="flex items-center space-x-4">
              ...
              <div>
                 <span className="block text-[10px] font-black text-gray-400 uppercase tracking-widest leading-none mb-1 font-sans">Expertise</span>
                 <span className="text-xl font-black text-gray-900 leading-none">{service.serviceType}</span>
              </div>
           </div>
        </div>
      </div>
                  ...
                  {service.catalogue.map((item, idx) => (
                    <div key={idx} className="space-y-4">
                      <div className="flex justify-between items-center bg-gray-50 p-4 rounded-2xl border border-gray-100">
                        <h4 className="font-black text-purple-600 uppercase tracking-widest">{item.level}</h4>
                        <div className="flex gap-2 items-center">
                          ...
                          {item.priceRate && (
                            <span className="text-xs font-black bg-purple-600 text-white px-4 py-1.5 rounded-full shadow-md flex items-center">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        {item.images.map((img, imgIdx) => (
                          <div key={imgIdx} className="aspect-square rounded-2xl overflow-hidden border-2 border-gray-100 shadow-sm hover:border-purple-200 transition-all">
                            <img 
                              src={resolveUrl(img)} 
            ...
            <div className="bg-white p-8 rounded-3xl shadow-2xl border border-orange-100 sticky top-24">
              <div className="flex items-baseline space-x-2 mb-6">
                <span className="text-3xl font-bold text-gray-900">
                  {bookingMode === 'complete' 
                    ? service.priceRange
                    : `₹${(service?.facility_details?.filter(c => selectedItems.includes(c.id || '')).reduce((sum, item) => sum + (item.priceRate || 0), 0) || 0).toLocaleString()}`
                  }
                </span>
              ...
              {bookingStatus === 'success' ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-500">The provider will contact you shortly.</p>
                  <button 
                ...
                <form onSubmit={handleBooking} className="space-y-5">
                  ...
                  <div className="bg-purple-50 p-4 rounded-2xl border border-purple-100 space-y-3">
                    <div className="flex justify-between items-center mb-1">
                      <label className="block text-xs font-black text-purple-600 uppercase tracking-widest font-sans">Booking Mode</label>
                      <div className="bg-white px-3 py-1 rounded-full border border-purple-200">
                        <span className="text-[10px] font-bold text-gray-500 uppercase tracking-tighter leading-none">Choose Package</span>
                      </div>
                    </div>
                    ...
                    {bookingMode === 'partial' && service.facility_details && (
                      <div className="space-y-2 mt-4 pt-4 border-t border-purple-100">
                        ...
                        <div className="grid grid-cols-1 gap-2 max-h-48 overflow-y-auto pr-2">
                          {service.facility_details.filter(c => c.priceRate && c.priceRate > 0).map(item => (
                          ...
                          {(!service.facility_details || service.facility_details.filter(c => c.priceRate && c.priceRate > 0).length === 0) && (
